...
export const BOT_ADVANCED_FUNCTIONS = {
  ...
  analyzeSentiment: (text) => {
    const positive = ['merci', 'super', 'génial', 'excellent', 'parfait', 'bravo', 'top', 'wow', 'incroyable']
    const negative = ['problème', 'erreur', 'bug', 'nul', 'mauvais', 'déçu', 'difficile']
  ...
  extractEntities: (text) => {
    ...
    // Pays
    const paysKeywords = ['france', 'guadeloupe', 'martinique', 'canada', 'usa', 'allemagne', 'espagne', 'italie']
    entities.pays = paysKeywords.filter(p => lowerText.includes(p))

    // Objectifs
    const objectifsKeywords = ['carrière', 'business', 'études', 'santé', 'créativité', 'finance', 'leadership', 'innovation']
    entities.objectifs = objectifsKeywords.filter(o => lowerText.includes(o))

    // Thèmes
    const themesKeywords = ['quiz', 'passeport', 'visa', 'boutique', 'bourse', 'emploi', 'formation']
  ...
  generatePersonalizedResponse: (query, userContext = {}) => {
    ...
    if (sentiment === 'positive') {
      ...
      response = '💪 Je comprends votre préoccupation. Laissez-moi vous aider ! '
      emoji = '🤝'
    }
    ...
    if (entities.themes.includes('passeport')) {
      response += 'Le Passeport de Réussite vous attend pour officialiser votre engagement ! '
    }
    if (entities.themes.includes('visa')) {
      response += 'Le VISA Universel ouvre les portes à des milliers d\'opportunités mondiales ! '
    }
  ...
  getSmartSuggestions: (query, conversationHistory = []) => {
    ...
    if (entities.themes.includes('quiz')) {
      suggestions.push('Voir tous les quiz', 'Commencer un quiz', 'Mes catégories préférées')
    } else if (entities.themes.includes('passeport')) {
      suggestions.push('Créer mon passeport', 'Voir des exemples', 'Partager mon passeport')
    } else if (entities.themes.includes('visa')) {
      suggestions.push('Obtenir mon VISA', 'Voir les opportunités', 'Bourses disponibles')
    } else if (entities.themes.includes('boutique')) {
      suggestions.push('Voir les boutiques', 'Produits recommandés', 'Boutiques par pays')
    } else {
      suggestions.push('Découvrir les quiz', 'Passeport de Réussite', 'VISA Universel', 'Boutiques Amazon')
    }
  ...
  saveConversation: (message, type) => {
    ...
    const history = JSON.parse(localStorage.getItem('bot_conversation_history') || '[]')
    history.push({
      message,
      type,
  ...
  quickSearch: (query) => {
    ...
    for (const [keyword, response] of Object.entries(knowledge.keywords)) {
      if (keyword.includes(lowerQuery) || response.toLowerCase().includes(lowerQuery)) {
        results.push({
          type: 'keyword',
          titre: keyword,
